...
// 좀처럼 풀리지 않는 의문이 있었다. 40억까지의 수중에서 제곱근보다 작은 소수중 가장 큰 소수는 63241이었는데
// 이것보다 큰 경우가 존재할때는 이전에 구해놓은 소수들로는 해결할수 없다는 것이었다. 오랜 고민끝에 알아냈는데
// 생각보다 쉬운것이었다. 결론적으로 그런일은 절대 일어날수 없다. 왜냐하면 주어지는 최대값은 40억이다. 40억의 제곱근인
// 63245까지의 소수들중 가장 큰 값은 상술하였듯 63241인데 이것보다 더 큰 소수가 필요한 경우는 제곱근이 63245보다 큰 경우이다.
// 당장 제곱근이 기존값보다 1 큰 63246이 된다고 치면 그 값은 40억하고도 5만이 넘어간다. 즉, 절대로 입력될리 없는 수이며
// 연산도중 얻는 모든 소수들중 가장 큰 수는 이미 확보된 63241이 되어 모든 수를 계산할수 있게 된다.
