...
// 자세한 설명은 주석으로 남긴다.
// P.S 처음 풀때 실수를 하나 했었는데 그 이유는 바이러스를 풀수 있는 위치 idx는 N보다 더 클수 있다.
// 그런데 바이러스를 고를때 idx가 아닌 N으로 했기 때문에 모든 경우를 다 골라볼수 없었기 때문이다.
// 따라서 운좋게 idx가 N보다 더 작은 경우는 모든 경우를 다 고를수 있었으나 상술했듯 idx가 더 큰 경우는
// 모든 경우를 다 골라볼수 없어 제대로 된 답이 도출되지 않았던 것이다.
...
void BackTrack(int prev, int num)
{
    ...
    // 여기서 idx가 아닌 N으로 해서 꽤나 헤맸다. 상술한대로 idx가 더 큰 경우가 존재하는데
    // N으로 잘못 구현하였을때 모든 경우를 다 골라볼수 없어 제대로 된 답이 나오지 않는다.
